...
class SendNotification {
   ...
   public function android($notificationRow,$data){
      ...
      if(empty($data['users']->android_device_id) || strlen($data['users']->android_device_id)<20){return false;}
      ...
      sendAndroid::send($data['users']->android_device_id,$notificationRow->title,$body);
   }
   public function ios($notificationRow,$data){


      if(empty($data['users']->ios_device_id) || strlen($data['users']->ios_device_id)<20){return false;}

      $view='admin.notification::template.'.$notificationRow->language.'.'.$notificationRow->type.'_'.$notificationRow->name;
      $body=view($view,$data)->render();

      sendIos::send($data['users']->ios_device_id,$notificationRow->title,$body);
   }
   public function email($notificationRow,$data){
      $view='admin.notification::template.'.$notificationRow->language.'.'.$notificationRow->type.'_'.$notificationRow->name;

      $subject=$notificationRow->title;
      $to_emal=$data['users']->email;
      SendEmail::send($to_emal,$subject,$view,$data);
   }
   public function sms($notificationRow,$data){

      $phone=(empty($data['users']->phone) || strlen($data['users']->phone)<5)? $data['users']->mobile:$data['users']->phone;
