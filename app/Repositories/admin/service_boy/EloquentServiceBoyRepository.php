<?php
namespace App\Repositories\admin\service_boy;

use Session;
use App\Models\ServiceBoy;
use App\Repositories\admin\service_boy\ServiceBoyContract;
...
class EloquentServiceBoyRepository implements ServiceBoyContract
{
    ...
    public function getByFilter($data)
    {
        ...
        $oResults =  ServiceBoy::with('users');
    ...
    public function getAllList($data = [])
    {
        ...
        if (canAccess('admin.service_company.allData')) {
            ...
            $oResults = $oResults->whereIn('service_company_id', current_user()->getServiceCompany());
        } elseif (canAccess('admin.service_company.userData')) {
        ...
        foreach ($oResults as $result) {
            $aResults[$result->id] = isset($result->users->first_name)?$result->users->first_name:'';
        }
        return $aResults;
    }
