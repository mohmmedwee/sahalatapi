<?php
namespace App\Repositories\admin\audit_track;

use Session;
use App\Models\AuditTrack;
use App\Repositories\admin\audit_track\AuditTrackContract;
...
class EloquentAuditTrackRepository implements AuditTrackContract
{
    ...
    public function getByFilter($data)
    {
        ...
        if (isset($data['from_date']) && !empty($data['from_date'])) {
            $oResults = $oResults->where('created_at', '>=', $data['from_date']);
        }

        if (isset($data['to_date']) && !empty($data['to_date'])) {
            $oResults = $oResults->where('created_at', '<=', $data['to_date']);
        }
