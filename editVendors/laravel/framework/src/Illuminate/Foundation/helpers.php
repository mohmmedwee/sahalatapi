...
if (! function_exists('view')) {
    ...
    function view($view = null, $data = [], $mergeData = [])
    {
        ...
        if(isset($data['request']) && isset($data['request']['ajaxRequest'])){


            $newData=[];
            if(isset($data['responseData'])){

                foreach($data['responseData'] as $responseKey){
                    if(isset($data[$responseKey])){
                        $newData[$responseKey]=(gettype($data[$responseKey]) == "object")? (array)$data[$responseKey]:$data[$responseKey];
                    }
                }
            }else{

                foreach($data as $key=>$oneData){
                    $newData[$key]=(gettype($oneData) == "object")? (array)$oneData:$oneData;
                }

            }
            return new \Illuminate\Http\JsonResponse((array)$newData, 200);
